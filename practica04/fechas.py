...
#aqui tines el dt que es la fecha y eliges como mostrarla

	if args.json:
		print json.dumps({fecha[0]:dt})

	if args.ascii:
		print fecha[0],dt

	if not (args.json and args.ascii):
		root = ET.Element(u"Instants")
		root.attrib={u"City":args.timezone}

		elemento = ET.SubElement(root,u"Instant")
		atributos = {u"date":str(dt),u"Ordinal":fecha[0]}
